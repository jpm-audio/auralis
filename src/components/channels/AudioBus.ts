...
import { ROUTING_OPTIONS_DEFAULT } from "../constants/RoutingOptions";
import { IRoutingHandler, IRoutingHandlerOptions } from "../routing/types";
import Parameter from "../parameters/Parameter";
import { eParameterEvent, eParameterType } from "../parameters/types";
 ...
 */
export default class AudioBus extends EventEmitter implements IAudioBus {
    /**
	 * Name of the bus
	 */
	 ...
	 */
    protected _routing: IRoutingHandler;
    /**
	 ...
	 * @returns {IRoutingHandler} - The routing handler of the bus
	 */
    public get routing(): IRoutingHandler {
	 ...
	 * @param {IAudioBusOptions} options - The options for the audio bus.
	 *  This includes the name, volume, mute state, and pan value.
	 * @param {Partial<IRoutingHandlerOptions>} routingOptions - The options for the routing handler.
	 *  This includes the owner of the routing handler.
	 *  The owner is the audio bus that will be used to create the routing handler.
	 */
    constructor(
        options: IAudioBusOptions,
        routingOptions?: Partial<IRoutingHandlerOptions>
    ) {
        super();

        this._name = options.name;
        this._volume = new Parameter({
            type: eParameterType.CONTINUOUS,
        ...
        this._volume.on(
            eParameterEvent.UPDATE,
            this._onVolumeParameterUpdate,
            this
        );
        this._onVolumeParameterUpdate(this._volume);

        this._pan.on(eParameterEvent.UPDATE, this._onPanParameterUpdate, this);
        this._onPanParameterUpdate(this._pan);
    }
    /**
	 * Create event info object
	 * @param {eAudioBusEvents} event - The event to create the info for
	 ...
	 * @returns {IAudioBusEventInfo} - The event info object
	 */
    protected _createEventInfo(
        event: eAudioBusEvents,
        previousValue: any,
        value: any,
	 ...
	 * @param {IAudioBus} destination - The bus to connect to
	 * @returns {IAudioBus | undefined} - The connected bus or undefined if the connection failed
	 */
    public connect(destination: IAudioBus): IAudioBus | undefined {
    ...
    /**
	 * Disconnect the bus from another bus
	 * @param {IAudioBus} destination - The bus to disconnect from
	 * @returns {boolean} - True if the disconnection was successful, false otherwise
	 */
    public createChildBus(options: IAudioBusOptions) {
    ...
    public destroy(): void {
        ...
        this._pan.off(eParameterEvent.UPDATE, this._onPanParameterUpdate, this);
    }
}
