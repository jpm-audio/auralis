...
import { eMarkerType, IMarkerLoopOptions, IMarkerOptions, IMarkerTransitionOptions, ITimelineClipDefinition } from "./types";

export default class Timeline extends AudioBus {
     ...
     */
    addAudioClip(audioClipOptions: IAudioClipOptions, startTime: number, timeLength?: number): Timeline;
    /**
     * Add an audio clip to the timeline to a specific point in time.
    ...
    addAudioClip(clipOrOptions: AudioClip | IAudioClipOptions, startTime: number, timeLength?: number): Timeline {
    ...
    addLoopMarker(markerOptions: IMarkerLoopOptions, startTime:number): Marker {
    ...
    addDestinationMarker(markerOptions: IMarkerOptions, startTime:number): Marker {
    ...
    protected _createMarker(type:eMarkerType, markerOptions: IMarkerOptions | IMarkerTransitionOptions, startTime:number): Marker {
        const id = `${type}-${generateId()}}`;
        let marker: Marker;

        switch (type) {
        case eMarkerType.LOOP:
            marker = new MarkerLoop(markerOptions as IMarkerLoopOptions);
