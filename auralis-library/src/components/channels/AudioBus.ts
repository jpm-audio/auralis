...
import { ROUTING_OPTIONS_DEFAULT } from "@/components/constants";
import { Parameter, ParameterEvent, ParameterType } from "@/components/parameters";
import { GLOBAL_CONFIG } from "@/core/AudioGlobalConfig";
import { RoutingHandler, type RoutingHandlerOptions } from "@/components/routing";
 ...
 */
export class AudioBus extends EventEmitter implements AudioBusInterface {
    /**
	 * Name of the bus
	 */
	 ...
	 * @param {AudioBusInterface} destination - The bus to connect to
	 * @returns {AudioBusInterface | undefined} - The connected bus or undefined if the connection failed
	 */
    public connect(destination: AudioBusInterface): AudioBusInterface | undefined {
    ...
    /**
	 * Disconnect the bus from another bus
	 * @param {AudioBusInterface} destination - The bus to disconnect from
	 * @returns {boolean} - True if the disconnection was successful, false otherwise
	 */
