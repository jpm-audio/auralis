...
import type { Playable, CurveFunction } from "@/components/types";
import { Curve } from "./Curve";
import { ParamAutomationEvent, type ParamAutomationOptions } from "./types";
import { Deferred } from "@/utils/deferred";
import type { AudioTicker } from "@/core";
 ...
 */
export class ParamAutomation extends EventEmitter implements Playable {
    /**
	 ...
	 */
    public curve: CurveFunction = Curve.linear;
    /**
    ...
    protected _update(ticker: AudioTicker): void {
        ...
        if (isComplete) {
            this.stop();
            this.emit(ParamAutomationEvent.COMPLETE, this);
            return;
        }
    }
    /**
    ...
    protected _updateValue(value: number): void {
        if (this._value !== value) {
            this._value = value;
            this.emit(ParamAutomationEvent.UPDATE, this);
        }
    }
    /**
    ...
    public async play(): Promise<ParamAutomation> {
        ...
        if (this._isPaused) {
            ...
            this.emit(ParamAutomationEvent.RESUME);
        } else {
            this._playbackDeferred = new Deferred<ParamAutomation>();
            this._elapsed = 0;
            this.emit(ParamAutomationEvent.PLAY, this);
        }
    ...
    public pause(): ParamAutomation {
        if (this.isPlaying && !this._isPaused) {
            ...
            this.emit(ParamAutomationEvent.PAUSE, this);
        }
    ...
    public stop(): ParamAutomation {
        if (this.isPlaying) {
            ...
            this.emit(ParamAutomationEvent.STOP, this);
        }
