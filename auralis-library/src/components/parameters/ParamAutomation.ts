...
import type { CurveFunction, Playable } from "@/components/types";
import { Curve } from "./Curve";
import {
    ParamAutomationEvents,
    type ParamAutomationOptions,
} from "./types";
import { Deferred } from "@/utils";
import type { AudioTicker } from "@/core/AudioTicker";
...
export class ParamAutomation extends EventEmitter implements Playable {
    ...
    protected _update(ticker: AudioTicker): void {
        ...
        if (isComplete) {
            this.stop();
            this.emit(ParamAutomationEvents.COMPLETE, this);
            return;
        }
    }
    /**
    ...
    protected _updateValue(value: number): void {
        if (this._value !== value) {
            this._value = value;
            this.emit(ParamAutomationEvents.UPDATE, this);
        }
    }
    /**
    ...
    public async play(): Promise<ParamAutomation> {
        ...
        if (this._isPaused) {
            ...
            this.emit(ParamAutomationEvents.RESUME);
        } else {
            this._playbackDeferred = new Deferred<ParamAutomation>();
            this._elapsed = 0;
            this.emit(ParamAutomationEvents.PLAY, this);
        }
    ...
    public pause(): ParamAutomation {
        if (this.isPlaying && !this._isPaused) {
            ...
            this.emit(ParamAutomationEvents.PAUSE, this);
        }
    ...
    public stop(): ParamAutomation {
        if (this.isPlaying) {
            ...
            this.emit(ParamAutomationEvents.STOP, this);
        }
